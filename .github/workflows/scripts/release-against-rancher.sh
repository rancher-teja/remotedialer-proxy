...
#
# Usage:
#   ./release-against-rancher.sh <path to rancher repo> <new remotedialer-proxy release>
#
# Example:
...
if ! PREV_REMOTEDIALER_VERSION_SHORT=$(yq -r '.remoteDialerProxyVersion' ./build.yaml | sed 's|.*+up||'); then
    echo "Unable to get remotedialer-proxy version from ./build.yaml. The content of the file is:"
    cat ./build.yaml
    exit 1
fi

if [ "$PREV_REMOTEDIALER_VERSION_SHORT" = "$NEW_REMOTEDIALER_VERSION_SHORT" ]; then
    echo "Previous and new remotedialer-proxy version are the same: $NEW_REMOTEDIALER_VERSION, but must be different"
    exit 1
fi
